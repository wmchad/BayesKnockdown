...
\begin{figure}
  \begin{center}
    \includegraphics[width=.6\textwidth]{simBarPlot.png}
  \end{center}
...
A more concrete example uses data from the National Institute of
...
The LINCS L1000 data captures gene expression levels of 1,000 genes in
...
a subset of the measured genes in the same experiments. The data has
...
we specify a prior of 0.0005, reflecting the belief that there are
very few relationships relative to the total possible number. Figure
\ref{fig:kdPlot} shows the range of values returned for the different
target genes.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=.6\textwidth]{kdPlot.png}
  \end{center}
  \caption{Bar plot showing the posterior probabilities of a
...
\begin{figure}
  \begin{center}
    \includegraphics[width=.6\textwidth]{diffExpPlot.png}
  \end{center}
